...
const IconSelector: React.FC<IconSelectorProps> = ({ 
  ...
  const validateIconName = (iconName: string): boolean => {
    ...
    // 检查是否是 Lucide 图标库中的图标（用显式注册表验证，避免整体导入 lucide）
    try {
      // 首先尝试直接匹配
      if (ICON_NAMES.has(iconName)) return true;
      
      // 如果包含连字符，尝试转换为 PascalCase
      if (iconName.includes('-')) {
        const pascalName = kebabToPascal(iconName);
        return ICON_NAMES.has(pascalName);
      }
      
      // 尝试首字母大写
      const capitalizedName = iconName.charAt(0).toUpperCase() + iconName.slice(1);
      return ICON_NAMES.has(capitalizedName);
    } catch {
      return false;
    }
  };
